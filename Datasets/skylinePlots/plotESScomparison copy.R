...
for (i in seq(1,length(temperatures),1)){
  ...
  # for (j in seq(1,100)){
    ...
    t <- fread(log[j], header=TRUE,select = c(1:2))
    ...
    new.dfname <- data.frame(method=method[[i]],posterior_ess = post_ess[[2]])
    if (first){
      dfname = new.dfname
      first = F
    }else{
...
ggsave(plot=p_speed, paste("../../Figures/ess_coda.pdf", sep=""),width=6, height=3)
