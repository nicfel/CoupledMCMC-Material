...
out_files = list.files(path="./out_mascotconverted/", pattern = "*.out", full.names=T)
...
plot(p.post)
ggsave(plot=p.post,paste("../../Figures/mascot_acceptance.pdf", sep=""),width=9, height=3)
...
plot(p.post)
ggsave(plot=p.post,paste("../../Figures/mascot_temperature.pdf", sep=""),width=9, height=3)
